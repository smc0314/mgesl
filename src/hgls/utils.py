...
def cal_length(triple, s_f, s_t, t, L=365, max_length=20, data_length=400, name=None):
    ...
    :return:
    """
    # if name == 'ICEWS05-15' or name == 'GDELT':    # rawrawrawrawrawrawrawrawrawrawrawrawrawraw  0
    #     if t-data_length < 0:
    #         s_f = s_f[:, 0:t+1]
    #         s_t = s_t[:, 0:t + 1]
    #     else:
    ...
    # entity, idx = np.unique(triple[:, 0], return_inverse=True)         # rawrawrawrawrawrawrawrawrawrawrawrawrawraw   0

    entity = np.arange(0, 7128) # ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14
    # entity = np.arange(0, 23033)  # ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18
    # entity = np.arange(0, 10488)  # ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15  1

    # s_f = s_f[entity, 0:t+1]    # 0到t时刻s的索引     rawrawrawrawrawrawrawrawrawrawrawrawrawraw  0
    # s_t = s_t[entity, 0:t+1]    # 0到t时刻s发生交互的时间   rawrawrawrawrawrawrawrawrawrawrawrawrawraw   0
    ...
    #     s_t = s_t[:, 0:t + 1]
    # else:
    #     s_f = s_f[:, t - max_length:t + 1]
    #     s_t = s_t[:, t - max_length:t + 1]
    # # /smc
    ...
    for i in range(len(entity)):
        # all_time = np.unique(s_t[i])[0:-1]      # rawrawrawrawrawrawrawrawrawrawrawrawrawrawrawraw   0

        # smc # ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14
        all_time = np.unique(s_t[i])
        if 365 in all_time:
            all_time = all_time[0:-1]
        # /smc

        # # smc # ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18
        # all_time = np.unique(s_t[i])
        # if 304 in all_time:
        #     all_time = all_time[0:-1]
        # # /smc

        # # smc # ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15     1
        # all_time = np.unique(s_t[i])
        # if 4017 in all_time:
        #     all_time = all_time[0:-1]
        # # /smc
        ...
        if len(all_time) == 0:
            ...
            # all_entity = np.unique(s_f[i])[0:-1]    # rawrawrawrawrawrawrawrawrawrawrawrawrawrawrawrawrawrawrawrawraw    0

            # smc # ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14ICEWS14
            all_entity = np.unique(s_f[i])
            # print(all_entity)
            if 82999 in all_entity:
                all_entity = all_entity[0:-1]
            # /smc

            # # smc # ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18ICEWS18
            # all_entity = np.unique(s_f[i])
            # if 299879 in all_entity:
            #     all_entity = all_entity[0:-1]
            # # /smc

            # # smc # ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15ICEWS05-15   1
            # all_entity = np.unique(s_f[i])
            # if 433921 in all_entity:
            #     all_entity = all_entity[0:-1]
            # # /smc
...
class myFloder(Dataset):
    ...
    def __getitem__(self, index):
        return [self.data[0][index]], self.data[1][index]   # self.data[0]包含了所有时间戳的子图， self.data[1]包含了所有时间戳的时间步（0，1，2，3）
...
class Collate():
    def __init__(self, num_nodes, num_rels, s_f, s_t, total_length, name='ICEWS14s', encoder='rgat', decoder='rgat',
        ...
        self.s_f = s_f    # 节点上次出现的编号
        self.s_t = s_t    # 节点上次出现的时间
    ...
    def collate_rel(self, data, data_length=400):
        triple, t = data[0]
        triple = np.array(triple[0])
        # print(triple.shape)
        if self.name in ['GDELT', 'ICEWS05-15']:     # ICEWS05-15 global图太大了，只提取当前时间步的前399个时间戳的子图
            e_dix = (self.g.edata['e_s'] < t) & (self.g.edata['e_s_'] > t-data_length)
            graph = dgl.edge_subgraph(self.g, e_dix, relabel_nodes=False)   # relabel_nodes=False表示这里的节点的label不重新进行编号
        else:
            graph = self.g
        # 元组扩增
        if triple.shape[-1] == 3:    # 因为静态知识图谱可能没有时间维度
        ...
        data_list = {}
        # 计算采样所需要的东西
        # sample_list: 当前时间戳下每个节点的前max_length个节点
        # time_list: 当前时间戳下每个节点的前max_length个节点所在的时间戳
        # sample_unique: 当前时间戳下前所有节点的前max_length的所有实体的集合
        # time_unique:  当前时间戳下前所有节点的前max_length的所有实体的时间戳的集合
        # list_length: 当前时间戳下前所有节点的的历史长度，比max_length大则取max_length，否则取最大长度
        ...
        if self.encoder in ['rgat', 'regcn'] or self.decoder in ['rgat','rgcn','rgat_r','rgat_r1']:
            ...
            old_n_id = sub_graph.ndata[dgl.NID]  # 新的子图中的节点在原始大图中的id
            # print("00000",old_n_id)
            # 为了进一步缩小规模，再根据时间采样子图, sub_d_graph， 用于进一步筛选边的条件：（子图中的边在time_unique中） 或 （子图中的边的头节点和尾节点都在sample_unique中）
        ...
        # 再根据e_t采样出sub_e_graph用于encoder  # smc 去掉时间边
...
# sample_neighbors 返回图中的所有节点，但只返回采样的边
# setdiff1d 返回在第一个数组中存在，但在第二个数组中不存在的元素
