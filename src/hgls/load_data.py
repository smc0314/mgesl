...
# @Software: PyCharm
import sys
# sys.path.append('/home/smc/mgesl')
sys.path.append('/public/home/detian/smc/mgesl')
import dgl
import numpy as np
...
def load_data(data_name='ICEWS14s'):
    ...
    total_times = range(len(total_data))  # 总共的时间步数量
    time_idx = np.zeros(len(time_num) + 1, dtype=int)
    time_idx[1:] = np.cumsum(time_num)  # 不同时刻的编号       [    0   131   322   490   626 ....]
    # print("21111111111111111111111111111111111111111111111111111111111")
    # print(len(time_num))
    # print(time_idx)
    # 计算不同数据集的起始 ID
    train_sid = 0
    ...
    # graph, data_dic = dgl.load_graphs('/home/smc/mgesl/src/hgls/graph_' + data_name)
    graph, data_dic = dgl.load_graphs('/public/home/detian/smc/mgesl/src/hgls/graph_' + data_name)
    graph = graph[0]
...
def load_data_list(data_name):
    ...
    train_list, _ = r_utils.split_by_time(data.train)
    valid_list, _ = r_utils.split_by_time(data.valid)
    test_list, _ = r_utils.split_by_time(data.test)
