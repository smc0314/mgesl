...
from utils import get_data_with_t
import argparse
...
def generate_graph(data, time, time_num, nodes_num=None, rel_nums=None, name='Sample'):
    ...
    :param data: 总的数据
    :param time: 时间戳序列
    :param time_num: 每个时间戳下entity的数量
    :param nodes_num: 节点的总的数量
    :param rel_nums: 关系的总的数量
    :param name: name
    :return:
    """
    u = []  # 记录每一个时间戳重新编码后的索引s
    ...
    for i, tim in enumerate(time):
        print(i)
        if name == 'Sample':    # False
            data_1 = get_data_with_t(data, tim)
        else:
            data_1 = data[i]
        src1, rel1, dst1 = data_1[:,[0,1,2]].transpose()
        uniq_v, edges = np.unique((src1, dst1), return_inverse=True)
        n_src1, n_dst1 = np.reshape(edges, (2, -1)) + idx[i]  # 给节点在新图中重新编码   其目的就是：原先的时间戳图中可能有节点编号比较大的节点，这里对它们进行去重并重新编号，并加上过去时间戳节点的数量来加以区分
        ...
        for j, tim in enumerate(time[i+1:]):
            ...
            rel.append(2*rel_nums*np.ones(2*len(un_entity), dtype=int))               # 时间类型的边   2*rel_nums是指的时间边
            ...
            rel_s.append((i+j+1)*np.ones(2*len(un_entity), dtype=int))               #  头实体发生时间  /smc 不应该这样说
    ...
    graph = dgl.graph((u, v))    # smc  节点对（包含了正反三元组）
    ...
    s_last_index = np.zeros((nodes_num, L), dtype=int)   # 记录每个时刻的索引    # smc  个人认为是记录上一时刻的
    ...
    for n_i, ei in enumerate(id):
        s_his[ei] = node_id_f[n_i]   # 当前节点id在去重之前的列表的首次出现的位置,也即该节点在全局图中的初始编码
        s_his_t[ei] = L                                # T+1时间
...
if __name__ == '__main__':
    ...
    time_num = [len(np.unique(da[:, [0, 2]])) for da in total_data]   # 每一个时间戳中涉及到的实体的个数
    total_times = range(len(total_data))                              # 总的时间戳数量
    save_path = './' + 'graph_' + args.data
    print(np.array(total_data).shape,total_times, time_num, num_nodes, num_rels)
