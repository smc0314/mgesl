...
from src.hgls.utils import comp_deg_norm
import math
...
class HRGNN(nn.Module):
    ...
    def forward(self, data_list, node_id_new=None, time_gap=None, device=None, mode='test'):
        ...
        # if self.encoder == 'ori':  # False
        #     pass
        # else:
        #     sub_e_graph = data_list['sub_e_graph'].to(device)
        #     pre_id = data_list['pre_e_nid']
        #     # print(pre_id)
        #     if self.encoder == 'regcn':   # True
        #         # print("关系数量", self.num_rels)
        #         # print("关系数量0", self.rel_embedding.weight.shape)
        #         # print(self.rel_embedding.weight[0:self.num_rels * 2].shape)
...
class TimeEncode(torch.nn.Module):  #time encoder type = 1
    ...
    def forward(self, ts):
        # print("ts", ts.unsqueeze(1))
        # print("basis_freq", self.basis_freq)
