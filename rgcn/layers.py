...
class UnionRGCNLayer(nn.Module):
    ...
    def forward(self, g, prev_h, emb_rel):
        ...
        # print(torch.arange(0, 1005, dtype=torch.long).cuda())
        # print(torch.arange(0, g.number_of_nodes(), dtype=torch.long).cuda())
        self.rel_emb = emb_rel
        # self.sub = sub
        # self.ob = ob
        if self.self_loop:
            # print("节点数", g.number_of_nodes())
            # print("入度", g.in_degrees().shape)
            # print(g.in_degrees(range(g.number_of_nodes())))

            # print("h", g.ndata['h'].shape)
            #loop_message = torch.mm(g.ndata['h'], self.loop_weight)
