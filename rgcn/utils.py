...
def build_all_graph(num_nodes, num_rels, input_list, use_cuda, gpu):
    def comp_deg_norm(g):
        in_deg = g.in_degrees(range(g.number_of_nodes())).float()
        in_deg[torch.nonzero(in_deg == 0).view(-1)] = 1
        norm = 1.0 / in_deg
        return norm
    all_triples = np.concatenate(input_list, axis=0)
    all_triples = all_triples[:, :3]
    src, rel, dst = all_triples.transpose()
...
def build_sub_graph_0(num_nodes, num_rels, input_list, use_cuda, gpu):
    def comp_deg_norm(g):
        in_deg = g.in_degrees(range(g.number_of_nodes())).float()
        in_deg[torch.nonzero(in_deg == 0).view(-1)] = 1
        norm = 1.0 / in_deg
    ...
    for i in range(1, len(input_list)):
        ...
        pre = pre[:, :3]
        src_pre, rel_pre, dst_pre = pre.transpose()
        src_pre, dst_pre = np.concatenate((src_pre, dst_pre)), np.concatenate((dst_pre, src_pre))
        rel_pre = np.concatenate((rel_pre, rel_pre + num_rels))

        now = now[:, :3]
        src_now, rel_now, dst_now = now.transpose()
        src_now, dst_now = np.concatenate((src_now, dst_now)), np.concatenate((dst_now, src_now))
        rel_now = np.concatenate((rel_now, rel_now + num_rels))

        pre_u = np.unique((src_pre, dst_pre))
        now_u = np.unique((src_now, dst_now))
        intersection = np.intersect1d(pre_u, now_u)
        src_insersection = intersection
        dst_intersection = intersection + num_nodes
        rel_intersection = np.ones(intersection.shape[0]) * num_rels * 2

        src = np.concatenate((src_pre, src_now + num_nodes, src_insersection))
        dst = np.concatenate((dst_pre, dst_now + num_nodes, dst_intersection))
        rel = np.concatenate((rel_pre, rel_now, rel_intersection))

        g = dgl.DGLGraph()
        g.add_nodes(num_nodes * 2)
        g.add_edges(src, dst)
        norm = comp_deg_norm(g)
        node_id = torch.arange(0, num_nodes*2, dtype=torch.long).view(-1, 1)
        g.ndata.update({'id': node_id, 'norm': norm.view(-1, 1)})
        g.apply_edges(lambda edges: {'norm': edges.dst['norm'] * edges.src['norm']})
        g.edata['etype'] = torch.LongTensor(rel)
